...
from keystone.common.sentry_rule_loader import (
...
class SentryRuleLoaderTestCase(unit.BaseTestCase):
    ...
    def test_missing_rules_section(self):
        ...
        self.assertRaises(RuleValidationError, load_rules_from_file, config_file)
    ...
    def test_rate_limit_not_dictionary(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'exception_type': 'TestError', 'rate_limit': 'not_dict'}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_rate_limit_missing_max_occurrences(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'exception_type': 'TestError', 'rate_limit': {'time_window': 300}}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_rate_limit_missing_time_window(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'exception_type': 'TestError', 'rate_limit': {'max_occurrences': 5}}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_rate_limit_max_occurrences_not_int(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'exception_type': 'TestError', 
                      'rate_limit': {'max_occurrences': 'five', 'time_window': 300}}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_rate_limit_time_window_not_number(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'exception_type': 'TestError', 
                      'rate_limit': {'max_occurrences': 5, 'time_window': 'five_minutes'}}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_valid_message_pattern(self):
        ...
        config_data = {
            'rules': [{'name': 'test_rule', 'message_pattern': 'Connection.*refused.*port\s+\d+'}]
        }
        config_file = self._create_temp_config(config_data)
    ...
    def test_complex_valid_rule(self):
        """Test validation succeeds with complex rule having multiple conditions."""
        config_data = {
            'rules': [{
                'name': 'complex_rule',
    ...
    def test_config_not_dictionary(self):
        ...
        self.assertRaises(RuleValidationError, load_rules_from_file, temp_file.name)
