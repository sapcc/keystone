...
class FederatedGroupCacheTest(unit.TestCase):
    ...
    def test_token_revocation_persists_event_and_invalidates_cache(
            self, mock_cache_notification, mock_audit_internal):
        """Test that token revocation both persists event and invalidates cache.

        Verifies that invalidate_user_cache_on_group_change:
        1. Creates a revocation event (PERSIST_REVOCATION_EVENT_FOR_USER)
        2. Invalidates the token cache
        """
        ...
        # Verify revocation event was persisted
        mock_audit_internal.assert_called_once_with(
            notifications.PERSIST_REVOCATION_EVENT_FOR_USER,
            self.user['id']
        )

        # Verify cache invalidation notification was sent
