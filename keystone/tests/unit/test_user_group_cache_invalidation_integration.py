...
class FederatedGroupCacheIntegrationTest(unit.TestCase):
    ...
    def test_complete_user_shadowing_workflow(self):
        ...
        with mock.patch.object(
                ...
                'invalidate_user_cache_on_group_change') as mock_revoke:
            ...
            mock_revoke.assert_called_with(user['id'])
    ...
    def test_multiple_users_concurrent_operations(self):
        ...
        original_revoke = PROVIDERS.assignment_api.invalidate_user_cache_on_group_change
        ...
        with mock.patch.object(
            ...
            # Each user should have one revocation (new membership)
    ...
    def test_expired_membership_removal_triggers_revocation(self):
        ...
        # Mock cleanup to simulate finding expired memberships
        with mock.patch.object(
    ...
    def test_role_assignments_reflect_group_changes(self):
        ...
        Verifies that after cache invalidation, querying role assignments
        returns the correct updated roles based on current group membership.
        """
        ...
        PROVIDERS.assignment_api.invalidate_user_cache_on_group_change(user['id'])

        # Verify member role via group2
    ...
    @mock.patch('keystone.notifications.Audit.internal')
    def test_token_revocation_event_persisted(self, mock_audit_internal):
        """Test that PERSIST_REVOCATION_EVENT_FOR_USER is called on group change.

        This integration test verifies that when a federated user's group
        membership changes during shadowing, a token revocation event is
        persisted to ensure old tokens are actually revoked.
        """
        ...
        # Shadow user with new group membership
        user = PROVIDERS.identity_api.shadow_federated_user(
            self.idp['id'],
            self.protocol['id'],
            user_dict,
        ...
        # Verify that PERSIST_REVOCATION_EVENT_FOR_USER was called
        mock_audit_internal.assert_any_call(
            notifications.PERSIST_REVOCATION_EVENT_FOR_USER,
            user['id']
        )

        mock_audit_internal.reset_mock()

        # Change group membership (add group2)
        user = PROVIDERS.identity_api.shadow_federated_user(
            self.idp['id'],
            self.protocol['id'],
            user_dict,
        ...
        # Verify revocation event was called again
        mock_audit_internal.assert_any_call(
            notifications.PERSIST_REVOCATION_EVENT_FOR_USER,
            user['id']
        )
