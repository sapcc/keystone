...
class V3TokenDataHelper(object):
    ...
    def _populate_is_admin_project(self, token_data):
        ...
        # CCloud: we need to support a fallback in the default domain during bootstrapping,
        # where only the default domain exists
        bootstrap_project_name = CONF.resource.bootstrap_project_name
        bootstrap_project_domain_name = CONF.resource.bootstrap_project_domain_name

        if not (admin_project_name and admin_project_domain_name):
            if not (bootstrap_project_name and bootstrap_project_domain_name):
        ...
        token_data['is_admin_project'] = (
            ...
            project['domain']['name'] == admin_project_domain_name) or
            (project['name'] == bootstrap_project_name and
             project['domain']['name'] == bootstrap_project_domain_name))
